...
| `GET /v4/leagues/{id}/managers/{id}/performance` | **Used** since 2026-09-08 — [documented](leagues.md#get-v4leaguesleagueidmanagersmanageridperformance), and read by `useManagerHistory` for the **gold title stars** on manager avatars. A manager's every season, which [Ranking](../pages/ranking.md) fakes out of `lp` for the current one — and the only route to **how often they have won the league** |
