...
cards, ten `8`s for the substitutions. Match-level entries (`pi: "0"`) use a
separate band of the same scale — `10`–`13` and `26` — identified on
2026-09-05; the app still drops them, deliberately.

`/v4/live/eventtypes` is a *different*, 621-entry catalogue of Kickbase's
scoring events (*Deadly Pass*, *Big Chance Created*). Nothing reads it, but it
is no longer a dead end: it is the lookup table for `eti` on the player
centre's `events[]`, whose per-event `p` values sum exactly to a player's
matchday points. It is also, despite the path, **not live** — seven polls
across a running matchday returned byte-identical payloads.
