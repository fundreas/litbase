 ...
 *
 * The squad is served **as it stands today** (there is no `?dayNumber=` for
 * it), so a simulated matchday shows *today's* eleven with that matchday's
 * points — the same gap the [duel detail page](../../docs/pages/duel-detail.md)
 * documents for past matchdays. Fine for building the UI, misleading if
 * mistaken for a real result, which is why the badge in the header says loudly
 * that the app is simulating.
 *
