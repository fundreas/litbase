...
/**
 * The app's navigation, as the last tab of a page's own bottom bar.
 *
 ...
 * thumb is already resting on the bottom bar.
 *
 * So the bar gets one more tab, on the right where the thumb is, and it is
 * **navigation rather than a view of the page** — the one entry in the row
 * that leaves. It is drawn as three dots and kept *thin*: the tabs it sits
 * beside are what the page is for, and this must not look like one of them or
 * take room from them. A left border separates the two kinds.
 *
 * `lg:hidden`, because from `lg` up the [sidebar](./NavSidebar.tsx) is on
 * screen permanently and there is nothing to reach for.
 *
 * ## The gesture
 *
  ...
  return (
    <li className="relative ml-1 shrink-0 border-l border-line pl-1 lg:hidden">
      {/*
        ...
        place: the bar carries `backdrop-blur`, and a `backdrop-filter` makes
        its element the containing block for `position: fixed` descendants — so
        a `fixed inset-0` child here would stretch to the bar's own box rather
        than to the window.

        `z-40` puts it over the page and under the bar, which
        [`BottomTabBar`](../ui/BottomTabBar.tsx) raises to `z-50` while this is
        open. The bar therefore stays lit with the sheet growing out of it, and
        the page behind recedes — and a tap on one of the page's own tabs still
        lands on the tab, which navigates and closes this by doing so.
      */}
      {isOpen &&
        createPortal(
          <div
      ...
      <button
        ...
        className={cn(
          ...
          'flex h-12 w-8 items-center justify-center rounded-xl',
          'transition-colors',
      ...
      {isOpen && (
        <nav
          ...
          aria-label="Seiten"
          className={cn(
            // Anchored to the dots and growing up and to the left, out of the
            // bar. `bottom-full` is this cell's top edge, so the first row
            // lands just above the thumb whatever the bar's padding is.
